...
class TestUnaryNode(UnaryTestCase):
    ...
    def test_attributes_are_copied_from_child_when_init(self):
        expected = AttributeList(self.schema.get_attributes("beta"), "beta").to_list()
...
class TestSelect(UnaryTestCase):
    ...
    def test_exception_when_first_attribute_in_condition_is_wrong(self):
        self.assertRaises(AttributeReferenceError, SelectNode, self.alpha, "a2=42")

    def test_exception_when_second_attribute_in_condition_is_wrong(self):
        self.assertRaises(AttributeReferenceError, SelectNode, self.alpha, "a1=a2")

    def test_exception_when_both_attributes_in_condition_are_wrong(self):
        self.assertRaises(AttributeReferenceError, SelectNode, self.alpha, "a2=a3")

    def test_exception_when_prefix_in_condition_is_wrong(self):
        self.assertRaises(AttributeReferenceError, SelectNode, self.alpha, "beta.a1=42")

    def test_exception_when_ambiguous_attributes(self):
        self.assertRaises(AttributeReferenceError, SelectNode, self.ambiguous, "d1<5")
...
class TestProject(UnaryTestCase):
    ...
    def test_exception_when_attribute_does_not_exist(self):
        self.assertRaises(AttributeReferenceError, ProjectNode, self.alpha, ["void"])

    def test_exception_when_attribute_no_longer_exists(self):
        child = ProjectNode(self.gamma, ["c1", "c2"])
        self.assertRaises(AttributeReferenceError, ProjectNode, child, ["c3"])

    def test_exception_when_prefix_is_wrong(self):
        self.assertRaises(InputReferenceError, ProjectNode, self.beta, ["alpha.c2"])

    def test_exception_when_ambiguous_attributes(self):
        self.assertRaises(AttributeReferenceError, ProjectNode, self.ambiguous, "d1")
...
class TestRename(TestUnaryNode):
    ...
    def test_exception_when_too_few_attributes(self):
        self.assertRaises(InputError, RenameNode, self.beta, None, ["a"], self.schema)

    def test_exception_when_too_many_attributes(self):
        self.assertRaises(
            InputError, RenameNode, self.beta, None, ["a", "b", "c"], self.schema
        )

    def test_exception_when_name_conflicts(self):
        self.assertRaises(
            InputError, RenameNode, self.beta, "alpha", ["a", "b"], self.schema
        )

    def test_when_ambiguous_attributes(self):
        self.assertRaises(
            InputError, RenameNode, self.beta, "beta", ["b1", "b1"], self.schema
        )
