...
class TestAssignment(TestSQL):
    ...
    def test_relation_with_attributes(self):
        ...
        expected = [
            "CREATE TEMPORARY TABLE newalpha(a, b, c) AS "
            "SELECT alpha.a1, alpha.a2, alpha.a3 FROM alpha"
        ]
        actual = self.translate(ra)
        self.assertEqual(expected, actual)
    ...
    def test_project(self):
        ...
        expected = [
            "CREATE TEMPORARY TABLE bilk(b1, b2) AS SELECT beta.b1, beta.b2 FROM beta"
        ]
        actual = self.translate(ra)
        self.assertEqual(expected, actual)
    ...
    def test_project_with_attributes(self):
        ...
        expected = [
            "CREATE TEMPORARY TABLE bilk(a, b) AS SELECT beta.b1, beta.b2 FROM beta"
        ]
        actual = self.translate(ra)
        self.assertEqual(expected, actual)
    ...
    def test_join(self):
        ...
        expected = [
            "CREATE TEMPORARY TABLE bound(a1, a2, a3, b1, b2, b3) AS "
    ...
    def test_join_with_attributes(self):
        ...
        expected = [
            "CREATE TEMPORARY TABLE bound(a, b, c, d, e, f) AS "
            "SELECT alpha.a1, alpha.a2, alpha.a3, beta.b1, beta.b2, "
            "beta.b3 FROM "
            "(SELECT alpha.a1, alpha.a2, alpha.a3 FROM alpha) AS alpha "
            "CROSS JOIN "
...
class TestJoin(TestSQL):
    ...
    def test_select_right(self):
        ...
        expected = [
            "SELECT delta.d1, delta.d2, gamma.g1, gamma.g2 FROM "
            "(SELECT delta.d1, delta.d2 FROM delta) AS delta "
            "CROSS JOIN "
    ...
    def test_join_with_rename(self):
        ...
        translation = self.translate(ra)
        expected = [
            "SELECT g.g1, g.g2, h.g1, h.g2 FROM "
    ...
    def test_join_with_rename_set(self):
        ...
        expected = [
            "SELECT DISTINCT g.g1, g.g2, h.g1, h.g2 FROM "
...
class TestSet:
    ...
    def test_select_simple(self):
        ra = "\\select_{{g1 = g2}} (gamma {operator} gammatwin);".format(
            operator=self.ra_operator
        )
        root_list = TreeBRD(self.grammar).build(instring=ra, schema=self.schema)
        ...
        expected = [
            ...
            "{operator} ALL "
            "SELECT gammatwin.g1, gammatwin.g2 FROM gammatwin) "
            "AS _{name} WHERE g1 = g2".format(operator=self.sql_operator, name=name)
        ]
        self.assertEqual(expected, actual)
    ...
    def test_project_left(self):
        ra = "\\project_{{g1, g2}} gammaprime {operator} gamma;".format(
            operator=self.ra_operator
        )
        ...
        expected = [
            ...
            "{operator} ALL "
            "SELECT gamma.g1, gamma.g2 FROM gamma) AS _{name}".format(
                operator=self.sql_operator, name=name
            )
        ]
        self.assertEqual(expected, actual)

    def test_rename_simple(self):
        ra = "\\rename_{{g}} (gamma {operator} gammatwin);".format(
            operator=self.ra_operator
        )
        ...
        expected = [
            "SELECT g.g1, g.g2 FROM "
            "(SELECT g1, g2 FROM (SELECT gamma.g1, gamma.g2 FROM gamma "
            "{operator} ALL "
            "SELECT gammatwin.g1, gammatwin.g2 FROM gammatwin) "
            "AS _{name}) AS g(g1, g2)".format(operator=self.sql_operator, name=name)
        ]
        self.assertEqual(expected, actual)
